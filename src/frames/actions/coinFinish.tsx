...
const coinFinishScreen: FrameHandler = async (c: CustomFrameContext) => {
  //// frame data ////
  ...
  if (coiner.points_balance < 2) {
    return c.res({
      image: (
        errorScreen(
          `you don't have enough $qp...`,
          `allowances coming soon™`,
          true
        )
      ),
  ...
  return c.res({
    ...
    imageAspectRatio: '1:1',
    intents: [
      <Button action='/coin/new'>new q</Button>,
      <Button action='/seeQs'>see qs</Button>,
      <Button.Reset>back</Button.Reset>,
...
export default coinFinishScreen